...
uint256 hashGenesisBlock("0xfaeaebb8e401dbd2c5af9a82a6594b9fd58f1fbd9f7b6aa950cbd10494192cd6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Speedcoin: starting difficulty is 1 / 2^12
...
int64 nChainStartTime = 1397300111;
...
{
    // Speedcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Speedcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Speedcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Speedcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 'd';
        pchMessageStart[3] = 'c';
        hashGenesisBlock = uint256("0x624854a97f2ecc6bcff214c2f93f4413009f6a738c84d764cd137bda598c46fc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "April 2014 - Speedcoin [SPD] is an open source crypto currency for world internet users";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = 2419556;
        block.nTime = 1397300777;
        
        if (fTestNet)
        {

            block.nNonce = 413204;
            block.nTime = 1397300111;
        }
...
unsigned char pchMessageStart[4] = { 0x25, 0xe4, 0xbb, 0xf2 }; // Speedcoin: increase each by adding 1 to bitcoin's value.
...
void static SpeedcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running SpeedcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SpeedcoinMiner terminated\n");
        throw;
    }
}
