...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SPD unit input in lnReqAmount, we generate the URI with SPD as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SPD, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
